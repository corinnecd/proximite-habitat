-- =============================================================================
-- Migration 0006 : supprimer la notification FICHE_AFFECTEE quand traitée
-- =============================================================================
-- Quand le commercial passe une fiche de AFFECTEE → ACCEPTEE ou REFUSEE,
-- la notification "Nouvelle fiche affectée" est supprimée de son espace.
-- =============================================================================
...
begin
  ...
  -- 2. AFFECTEE → notifier le commercial
  if p_new_status = 'AFFECTEE' and p_assigned_to is not null then
    -- Supprimer d'abord une éventuelle ancienne notification d'affectation
  ...
  -- 3. ACCEPTEE ou REFUSEE depuis AFFECTEE
  --    → supprimer la notification FICHE_AFFECTEE du commercial (fiche traitée)
  --    → notifier le prospecteur
  if p_new_status in ('ACCEPTEE', 'REFUSEE') and v_old_status = 'AFFECTEE' then
    -- Supprimer la notification "fiche affectée" du commercial : fiche traitée
    delete from public.notifications
    where fiche_id = p_fiche_id and type = 'FICHE_AFFECTEE';
  end if;
  ...
  if p_new_status = 'ACCEPTEE' and v_fiche.created_by is not null and v_fiche.created_by <> v_uid then
    ...
    values (v_fiche.created_by, v_org, 'VENTE_REALISEE', '🎉 Vente réalisée !',
      'Votre fiche ' || coalesce(v_fiche.reference, '') || ' a abouti à une vente. Total : ' || v_ventes_count || ' vente(s).',
      p_fiche_id);
