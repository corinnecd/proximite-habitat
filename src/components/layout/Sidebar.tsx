...
function SectionLabel({ label }: { label: string }) {
  return (
    <p className="px-4 pt-5 pb-1 text-[10px] font-bold tracking-widest uppercase text-[#86868b] select-none">
      {label}
    </p>
  );
}
...
export function Sidebar() {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full relative overflow-hidden">
      ...
      <div className="pointer-events-none absolute -top-16 -right-16 w-40 h-40 rounded-full bg-[#F97316] opacity-[.04] blur-2xl" />
      ...
      <div className="px-5 py-4 border-b border-[#ececec]">
        <Link href="/" className="flex items-center gap-3" onClick={close}>
          ...
          <div>
            ...
            <p className="text-[10px] font-semibold text-[#86868b] tracking-widest">CONSEIL</p>
          </div>
        </Link>
        {!isDG && organizationName && (
          <div className="mt-2 px-2 py-1 rounded-md bg-[#F97316]/10 border border-[#F97316]/20">
            <p className="text-[11px] font-semibold text-[#EA580C] text-center truncate">{organizationName}</p>
          </div>
        )}
      </div>
        ...
        {profileLoading ? (
          ...
          <div className="space-y-1 animate-pulse">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="px-4 py-2.5"><div className="h-4 rounded bg-black/5" style={{ width: `${60 + (i % 3) * 20}%` }} /></div>
            ))}
            <div className="px-4 pt-4 pb-1"><div className="h-2 w-20 rounded bg-black/5" /></div>
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={`b${i}`} className="px-4 py-2.5"><div className="h-4 rounded bg-black/5" style={{ width: `${50 + (i % 2) * 25}%` }} /></div>
            ))}
          </div>
        ) : (
          <>
      ...
      {/* Footer utilisateur */}
      <div className="px-3 py-3 border-t border-[#ececec] space-y-0.5">
        {profile ? (
          <>
          ...
          <>
            <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl">
              ...
              <div className="flex-1 space-y-1.5">
                <div className="h-3.5 w-28 bg-black/5 rounded animate-pulse" />
                <div className="h-2.5 w-16 bg-black/5 rounded animate-pulse" />
              </div>
            </div>
            <div className="flex items-center gap-3 px-4 py-2.5">
              <div className="h-4 w-4 bg-black/5 rounded animate-pulse" />
              <div className="h-3.5 w-24 bg-black/5 rounded animate-pulse" />
            </div>
          </>
        )}
      </div>
  ...
  return (
    <>
      ...
      <aside className={cn(
        "lg:hidden fixed inset-y-0 left-0 z-50 w-72 bg-[#fafafa] border-r border-[#ececec] transform transition-transform duration-300",
      ...
      <aside className="hidden lg:flex lg:w-72 lg:flex-col lg:fixed lg:inset-y-0 bg-[#fafafa] border-r border-[#ececec]">
        {sidebarContent}
      </aside>
    </>
  );
