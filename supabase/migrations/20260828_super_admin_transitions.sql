-- 20260828_super_admin_transitions.sql
-- Bug : le role SUPER_ADMIN n'apparaissait dans AUCUNE branche de la matrice de
-- transitions. Toute tentative levait « Transition X -> Y non autorisee pour le
-- role SUPER_ADMIN », alors que ce role dispose des pleins droits partout
-- ailleurs (edition de fiche, date de RDV, parcours, gestion des utilisateurs).
--
-- Correctif : SUPER_ADMIN est ajoute partout ou DIRECTION est deja autorise.
-- La signature ne change pas (5 arguments, p_montant_ht inclus) : un simple
-- CREATE OR REPLACE suffit, sans DROP ni re-GRANT.
...
begin
  ...
  v_allowed := case
    ...
    when v_fiche.status = 'BROUILLON'         and p_new_status = 'SOUMISE'
      then v_role in ('PROSPECTEUR', 'CHEF_EQUIPE', 'COMMERCIAL', 'ADMIN', 'DIRECTION', 'SUPER_ADMIN')

    -- SOUMISE → VALIDEE
    when v_fiche.status = 'SOUMISE'           and p_new_status = 'VALIDEE'
      then v_role in ('ADMIN', 'DIRECTION', 'SUPER_ADMIN')
    -- SOUMISE → BROUILLON
    when v_fiche.status = 'SOUMISE'           and p_new_status = 'BROUILLON'
      then v_role in ('ADMIN', 'DIRECTION', 'PROSPECTEUR', 'CHEF_EQUIPE', 'SUPER_ADMIN')
    ...
    when v_fiche.status = 'AFFECTEE'          and p_new_status = 'RETRACTATION'
      then v_role in ('ADMIN', 'DIRECTION', 'COMMERCIAL', 'SUPER_ADMIN')
    when v_fiche.status = 'AFFECTEE'          and p_new_status = 'ACCEPTEE'
      then v_role in ('ADMIN', 'DIRECTION', 'COMMERCIAL', 'SUPER_ADMIN')
    when v_fiche.status = 'AFFECTEE'          and p_new_status = 'REFUSEE'
      then v_role in ('ADMIN', 'DIRECTION', 'COMMERCIAL', 'PROSPECTEUR', 'CHEF_EQUIPE', 'SUPER_ADMIN')
