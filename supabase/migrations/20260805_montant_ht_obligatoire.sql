-- 20260805_montant_ht_obligatoire.sql
-- #5 de l'audit : rendre le montant HT obligatoire pour passer une fiche en ACCEPTEE.
--
-- Avant : `transition_fiche` passait la fiche en ACCEPTEE, puis le client faisait un
-- UPDATE séparé pour montant_ht. Une fiche pouvait donc rester ACCEPTEE avec
-- montant_ht = NULL (réseau coupé, appel direct à l'API) → CA faussé partout.
--
-- Après : le montant est un paramètre de la transition, contrôlé et écrit dans le
-- même UPDATE. L'ancienne signature à 4 arguments est supprimée pour éviter toute
...
begin
  ...
  -- Acceptation : montant HT obligatoire et strictement positif.
  -- Accepte soit le paramètre, soit un montant déjà enregistré sur la fiche.
  if p_new_status = 'ACCEPTEE' and coalesce(p_montant_ht, v_fiche.montant_ht, 0) <= 0 then
    raise exception 'Le montant HT du contrat est obligatoire pour accepter une fiche';
  end if;
  ...
  set status      = p_new_status,
      ...
      montant_ht  = case when p_new_status = 'ACCEPTEE' then coalesce(p_montant_ht, montant_ht) else montant_ht end,
      updated_at  = now()
  where id = p_fiche_id
...
-- L'ancienne signature doit disparaître : deux surcharges rendraient l'appel
-- PostgREST à 4 arguments nommés ambigu.
drop function if exists public.transition_fiche(uuid, public.fiche_status, text, uuid);

revoke all on function public.transition_fiche(uuid, public.fiche_status, text, uuid, numeric) from public;
grant execute on function public.transition_fiche(uuid, public.fiche_status, text, uuid, numeric) to authenticated;
