-- =============================================================================
-- Migration : ajout du statut RDV_A_REPRENDRE
-- Permet au commercial de signaler un client absent → le référent reprend RDV
-- =============================================================================

-- 1. Ajout de la valeur à l'enum fiche_status
alter type public.fiche_status add value if not exists 'RDV_A_REPRENDRE';

-- 2. Mise à jour de la fonction transition_fiche
...
begin
  ...
  v_allowed := case
    ...
    when v_fiche.status = 'AFFECTEE'          and p_new_status = 'SOUMISE'
      then v_role = 'ADMIN'
    -- AFFECTEE → RDV_A_REPRENDRE (client absent)
